...
export const actions = {
  async getInitialData({ commit }) {
    const url =
      'https://api.currencyapi.com/v3/latest?apikey=mECXec0IwIt52Ik75EuwAHKMDCfMQNEB9oHEgi23'
    const options = {
      method: 'GET',
    }
  ...
  async getInitialHistorical({ commit }) {
    ...
    for (let i = 1; i <= 12; i++) {
      const url = `https://api.currencybeacon.com/v1/historical?base=USD&date=2022-${
        i >= 10 ? i : `0${i}`
      }-01&api_key=a54189304372aeabe2941ffaecf3ea97`
      const ratesResponse = await fetch(url, options)
      if (ratesResponse.ok) {
        const json = await ratesResponse.json()
        commit('pushhistoricalRates', json.response)
      } else {
        alert('Ошибка API latestRates: ' + ratesResponse.status)
      }
    }
  },
  ...
  async getHistoricalData({ commit }, newBase) {
    ...
    for (let i = 1; i <= 12; i++) {
      const url = `https://api.currencybeacon.com/v1/historical?base=${newBase.trim()}&date=2022-${
        i >= 10 ? i : `0${i}`
      }-01&api_key=a54189304372aeabe2941ffaecf3ea97`
      const ratesResponse = await fetch(url, options)
      if (ratesResponse.ok) {
