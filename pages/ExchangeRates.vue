<template>
  <v-container
    class="container px-4 d-flex justify-center"
  ...
  methods: {
    getRates() {
      this.$refs.observer.validate().then((result) => {
        if (
          this.$store.state.latestRates.base.toUpperCase() !==
          this.baseCurrency.toUpperCase()
        ) {
          // is valid, different currency, this currency exists
          if (
            result &&
            this.$store.state.latestRates.jsonData.data[
              this.baseCurrency.toUpperCase()
            ]
          ) {
            this.requestRates()
    ...
    async requestRates() {
      let url1 =
        'https://api.currencyapi.com/v3/latest?apikey=mECXec0IwIt52Ik75EuwAHKMDCfMQNEB9oHEgi23&base_currency='
      ...
      if (latestRatesResponse.ok) {
        ...
        this.$store.commit(
          'setlatestRates',
          json,
          this.baseCurrency.toUpperCase().trim()
        )
      } else {
